...
(module+ test
  ...
  (let* ([g (dataframe (list (series '("a" "a" "a" "b" "b" "b") #:name "g")
                             (series (list 3 1 2 5 polars-null 4) #:name "v")
                             (series (list #t polars-null #f polars-null #t #f) #:name "b")))]
         [out (~> g (group-by "g")
                  (agg (alias (~> "v" shift (sort #:nulls-last #t) first) "f")
                       (alias (~> "v" shift (sort #:nulls-last #t) last) "l")
                       (alias (~> "v" shift (sort #:descending #t) first) "fd")
                       (alias (~> "b" shift (sort #:descending #t #:nulls-last #t) first) "bf"))
                  (sort "g"))]
         [over-g (lambda (e) (~> g (select (alias (over e "g") "s")) (column "s")))])
    (check-equal? (column out "f") '(1 5))
    (check-equal? (column out "l") (list polars-null polars-null))
    (check-equal? (column out "fd") (list polars-null polars-null))
    (check-equal? (column out "bf") '(#t #t))
    (check-equal? (over-g (~> "v" diff (sort #:nulls-last #t)))
                  (list -2 1 polars-null polars-null polars-null polars-null))
    (check-equal? (over-g (~> "v" diff (sort #:descending #t)))
                  (list polars-null 1 -2 polars-null polars-null polars-null))
    (check-equal? (over-g (sort "b" #:descending #t #:nulls-last #t))
                  (list #t #f polars-null #t #f polars-null)))
  (let ([nulls (~> (dataframe (list (series (list polars-null polars-null) #:name "n" #:dtype 'int64)))
                   (with-columns (cast "n" 'null)))])
    (check-equal? (~> nulls (select (sort "n" #:descending #t #:nulls-last #t)) (column "n"))
                  (list polars-null polars-null)))

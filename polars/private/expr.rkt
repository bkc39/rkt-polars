...
(module+ test
  ...
  (for ([_ (in-range 100)])
    (contracted:expr-sort sort-g #:nulls-last #t)
    (contracted:expr-sort-by sort-g #:by sort-h #:nulls-last #t #:maintain-order #t))
  (settle!)
  (check >= (- (expr-drop-count) sort-drops-before) 150))
